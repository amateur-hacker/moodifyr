...
const DateRangePicker = () => {
  const { isDateRangePickerDisabled, setStartDate, setEndDate } = use(
  ...
  >["onChange"] = (value) => {
    ...
    if (value.start) {
      const formattedStart = startOfDay(
        format(value.start.toDate(timeZone), "MM dd yyyy"),
      );
      setStartDate(formattedStart);
    }
    if (value.end) {
      const formattedEnd = endOfDay(
        format(value.end.toDate(timeZone), "MM dd yyyy"),
      );
